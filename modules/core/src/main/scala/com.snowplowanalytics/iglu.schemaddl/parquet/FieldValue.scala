...
object FieldValue {
  ...
  def cast(field: Field)(value: Json): CastResult =
    value match {
      case Json.Null =>
        field match {
          case Field(_, _, Type.Nullability.Optional) => NullValue.validNel
          case Field(_, Type.Json, _) => JsonValue(value).validNel
          case Field(_, fieldType, _) => WrongType(value, fieldType).invalidNel
        }
      case nonNull =>
  ...
  private [parquet] def castNonNull(fieldType: Type, value: Json): CastResult = {
    fieldType match {
      ...
      case Type.Array(element, nullable) =>
        value.asArray match {
          case Some(values) => values
            .toList
            .map {
              case Json.Null =>
                (nullable, element) match {
                  case (Type.Nullability.Optional, _) => NullValue.validNel
                  case (_, Type.Json) => JsonValue(Json.Null).validNel
                  case (Type.Nullability.Required, _) => WrongType(Json.Null, element).invalidNel
                }
  ...
  private def castObject(subfields: List[Field])(jsonObject: Map[String, Json]): CastResult =
    subfields.map { f =>
        jsonObject.get(f.name) match {
          ...
          case None =>
            f.nullable match {
              case Type.Nullability.Optional => (Field.normalizeName(f) -> NullValue).validNel
