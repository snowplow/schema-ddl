...
class MigrationSpec extends Specification { def is = s2"""
  ...
  def e1 = {
    ...
    Migration.buildMigration(segment) must beEqualTo(migration)
  }
  ...
  def e2 = {
    ...
    val comp1 = Migration.buildMigration(SchemaList.Segment(NonEmptyList.of(initialSchema, secondSchema))) must beEqualTo(migration1)
    ...
    val comp2 = Migration.buildMigration(SchemaList.Segment(NonEmptyList.of(initialSchema, secondSchema, thirdSchema))) must beEqualTo(migration2)
    ...
    val comp3 = Migration.buildMigration(SchemaList.Segment(NonEmptyList.of(secondSchema, thirdSchema))) must beEqualTo(migration3)
  ...
  def e3 = {
    ...
    Migration.buildMigration(SchemaList.Segment(NonEmptyList.of(initialSchema, secondSchema))) must beEqualTo(migration)
  }
