...
object Migration {
  ...
  /**
    ...
    */
  case class SchemaList private(schemas: NonEmptyList[IgluSchema]) extends AnyVal

  def getFullSource(matrix: NonEmptyList[SchemaList], last: Option[SchemaVer.Full]): SchemaList =
    last match {
      case Some(version) =>
        matrix
  ...
  implicit val schemaOrdering = implicitly[Order[(Int, Int)]].contramap[IgluSchema] {
    s => (s.self.schemaKey.version.revision, s.self.schemaKey.version.addition)
  }

  implicit val schemaMapOrdering = implicitly[Order[(Int, Int)]].contramap[SchemaMap] {
   ...
   */
  def buildMigration(source: SchemaList): Migration = {
  ...
  def buildMigrationMatrix(schemas: NonEmptyList[IgluSchema]): NonEmptyList[SchemaList] =
    schemas
      .groupByNem { case SelfDescribingSchema(SchemaMap(SchemaKey(v, n, _, SchemaVer.Full(model, _, _))), _) => (v, n, model) }
      .toNel
      .flatMap { case (_, s) =>
        val matrix = buildMatrix(s.toList).map { case (_, _, ss) => SchemaList(ss) }
