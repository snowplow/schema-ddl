...
object FlatData {
  ...
  def flatten(data: Json, source: SchemaList): List[String] =
    get(source).map(_._1.forData).map(pointer => getPath(pointer, data))


  def get(source: SchemaList): List[(SchemaPointer, Schema)] = {
    val origin = FlatSchema.build(source.schemas.head.schema)
    val originColumns = FlatSchema.order(origin.subschemas)
    val addedColumns = Migration.buildMigration(source).diff.added  // TODO: still no order?!
